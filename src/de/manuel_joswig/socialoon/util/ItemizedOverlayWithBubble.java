...
import de.manuel_joswig.socialoon.util.ISafeCanvas;
...
public class ItemizedOverlayWithBubble<Item extends OverlayItem> extends ItemizedIconOverlay<Item> {
	...
	public ItemizedOverlayWithBubble(final Context context, final List<Item> aList, final Drawable defaultMarker,
			final MapView mapView, final InfoWindow bubble, final Activity mapActivity) {
		...
		if (bubble != null){
			...
			if (layoutResId == 0){
				layoutResId = context.getResources().getIdentifier("layout/bonuspack_bubble_black", null, packageName);
				if (layoutResId == 0)
					Log.e(BonusPackHelper.LOG_TAG, "ItemizedOverlayWithBubble: layout/bonuspack_bubble_black not found in "+packageName);
			}
			mBubble = new DefaultInfoWindow(layoutResId, mapView, mapActivity);
		}
		mItemWithBubble = null;
	}

	public ItemizedOverlayWithBubble(final Context context, final List<Item> aList, final Drawable defaultMarker,
			final MapView mapView, final Activity mapActivity) {
		this(context, aList, defaultMarker, mapView, null, mapActivity);
	}
	...
	public BoundingBoxE6 getBoundingBoxE6(){
		...
		for (Item item:mItemList){
			points.add(new GeoPoint(item.getPoint().getLatitude(), item.getPoint().getLongitude()));
		}
	...
	public synchronized void drawSafe(final ISafeCanvas canvas, final MapView mapView, final boolean shadow) {
		...
		for (int i = size; i >= 0; i--) {
			...
			if (item != mItemWithBubble){
		        ...
		        onDrawItem(canvas.getSafeCanvas(), item, mCurScreenCoords, mapView.getMapOrientation());
			}
		}
		//draw focused item last:
		if (mItemWithBubble != null){
	        pj.toPixels(mItemWithBubble.getPoint(), mCurScreenCoords);
	        onDrawItem(canvas.getSafeCanvas(), (Item)mItemWithBubble, mCurScreenCoords, mapView.getMapOrientation());
		}
    }
